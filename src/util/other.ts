import { compressCardData } from 'src/util';
...
export function getShareUrl(value: string) {
    if (value === '') return '';
    const normalizedCard = { ...JSON.parse(value), artData: '', backgroundData: '' };
    const condensedCard = JSON.stringify(compressCardData(normalizedCard));
    if (typeof condensedCard !== 'string') return '';
    let searchParams = new URLSearchParams(window.location.search);
    searchParams.set('data', value);
